...
function updateChart(newValue) {
    // Create a new arc generator with the updated value
    const updatedArc = d3.arc()
        .innerRadius(chartRadius - 20)
        .outerRadius(chartRadius)
        .startAngle(-Math.PI / 2)
        .endAngle(-Math.PI / 2 + (newValue / max) * Math.PI);

    // Update the value arc with animation
    ...
    // Calculate the angle for the pointer based on the new value
    const pointerAngle = -Math.PI / 2 + (newValue / max) * Math.PI;

    // Update the pointer with animation
